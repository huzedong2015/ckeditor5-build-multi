...
export default class MultirootEditor extends Editor {
    ...
    constructor( sourceElements, config ) {
        ...
        if ( this.config.get( 'initialData' ) === undefined ) {
            ...
            for ( const element of sourceElements ) {
				const rootName = element.dataset.id;
                initialData[ rootName ] = getDataFromElement( element );
            }
		...
		sourceElements.forEach((el) => {
			this.model.document.createRoot( '$root', el.dataset.id );
		});
    ...
    destroy() {
        ...
        return super.destroy()
            .then( () => {
				editablesNames.forEach((rootName) => {
...
class MultirootEditorUI extends EditorUI {
    ...
    _initPlaceholder() {
        ...
        for ( const editable of this.view.editables ) {
            ...
            const placeholderText = sourceElement.getAttribute( 'placeholder' ) || editor.config.get( 'placeholder' );
...
class MultirootEditorUIView extends EditorUIView {
    ...
    constructor( locale, editingView, editableElements ) {
        ...
        for ( const element of editableElements ) {
            const editable = new InlineEditableUIView( locale, editingView, element );

            editable.name = element.dataset.id;
...
MultirootEditor.builtinPlugins = [
	Essentials, Paragraph, Heading, Bold, Italic, List, Link, BlockQuote, Image, ImageCaption,
            ImageStyle, ImageToolbar, ImageUpload, Table, TableToolbar, MediaEmbed, FindAndReplace, AutoSave
];
